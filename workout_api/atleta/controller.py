...
async def post(db_session: DatabaseDependency, atleta_in: AtletaIn = Body(...)):
    ...
    if not categoria:
        raise HTTPException(
            ...
            detail=f"Categoria não encotrada: {categoria_nome}",
        )
    ...
    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"centro treinamento não encotrada: {centro_treinamento_nome}",
        )
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Ocorreu um erro ao inserir os dados no banco dedos",
        )
...
@router.delete(
    "/{id}",
    summary="deletar uma atleta pelo id",
